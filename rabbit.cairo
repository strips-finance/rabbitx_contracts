...
@view
func get_total_deposited{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(trader: felt) -> (
...
@view
func get_total_withdrawn{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(trader: felt) -> (
...
func acknowledge_deposits{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
   ack_len: felt, ack: felt*
    ...
    local deposit_not_found: DepositInfo* = new DepositInfo(pending_index=NOT_FOUND_INDEX,
                                                            processed=0);
...
func populate_index_dict{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    local deposit_info: DepositInfo* = new DepositInfo(pending_index=pending_index,
                                                       processed=0);
...
func do_acknowledge_deposits{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    if (pending_index != NOT_FOUND_INDEX) {
        ...
        local updated_deposit_info: DepositInfo* = new DepositInfo(pending_index=pending_index,
                                                                   processed=1);
...
func pack_pending{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(dict_start: DictAccess*, dict_end: DictAccess*
) -> (dict_start: DictAccess*, dict_end: DictAccess*) {
    alloc_locals;
    let (local first_p) = first_pending_deposit.read();
    let (local num_p) = num_pending_deposits.read();
    ...
    let (local num_zero_slots, local num_non_zeros, local dict_start, local dict_end) = scan_pending(first_p, first_p+num_p, 0, zero_slots, 0, non_zeros, dict_start, dict_end);
    ...
    if (num_non_zeros == 0) {
        ...
        if (num_zero_slots != 0) {
            ...
            rewrite_pending(num_zero_slots=num_zero_slots, next_zero_slot=0, zero_slots=zero_slots,
                num_non_zeros=num_non_zeros, next_non_zero=num_non_zeros-1, non_zeros=non_zeros);
        }
    }
    return(dict_start, dict_end);
}
...
func scan_pending{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
   next_pending: felt, stop_pending: felt, next_zero_slot: felt, zero_slots: felt*, next_non_zero: felt, non_zeros: felt*, dict_start: DictAccess*, dict_end: DictAccess*
) -> (num_zero_slots: felt, num_non_zeros: felt, dict_start: DictAccess*, dict_end: DictAccess*) {
    alloc_locals;
    if (next_pending == stop_pending) {
        return(num_zero_slots=next_zero_slot, num_non_zeros=next_non_zero, dict_start=dict_start, dict_end=dict_end);
    }
    ...
    if (processed == 1) {
        ...
        return scan_pending(next_pending+1, stop_pending, next_zero_slot+1, zero_slots, next_non_zero, non_zeros, dict_start, dict_end);
    } else {
        // this pending slot is not marked, so is still to be processed
        assert non_zeros[next_non_zero] = next_pending;
        return scan_pending(next_pending+1, stop_pending, next_zero_slot, zero_slots, next_non_zero+1, non_zeros, dict_start, dict_end);
    }
}

func rewrite_pending{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
   num_zero_slots: felt, next_zero_slot: felt, zero_slots: felt*, num_non_zeros: felt, next_non_zero: felt, non_zeros: felt*
) {
    alloc_locals;
    ...
    if (zero_slot_comes_after_first_non_zero == TRUE) {
        ...
        let (zero_slot_comes_before_last_non_zero) = SafeCmp.is_lt_unsigned(next_zero, last_non_zero);
        if (zero_slot_comes_before_last_non_zero == TRUE) {
            ...
            if (any_non_zeros_left == TRUE) {
                return rewrite_pending(num_zero_slots, next_zero_slot+1, zero_slots, num_non_zeros, next_non_zero-1, non_zeros);
            }
            tempvar syscall_ptr = syscall_ptr;
            tempvar pedersen_ptr = pedersen_ptr;
            tempvar range_check_ptr = range_check_ptr;
...
func sum_withdrawals{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(
    withdrawals_len: felt, withdrawals: Withdrawal*, partial_sum: felt, dict_start: DictAccess*, dict_end: DictAccess*
    ...
    if (already_sent == FALSE) {
        ...
        return sum_withdrawals(withdrawals_len - 1, withdrawals + Withdrawal.SIZE, partial_sum + withdrawals[0].amount, dict_start, dict_end);
    }
    return sum_withdrawals(withdrawals_len - 1, withdrawals + Withdrawal.SIZE, partial_sum, dict_start, dict_end);
}
...
func do_withdraw{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    send_message_to_l1(
        to_address=l1_address, payload_size=payload_size, payload=message_payload
    );
    return();
}
...
func populate_payload{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    return populate_payload(payload=payload+2, payload_len=payload_len+2, dict_start=dict_start+DictAccess.SIZE, dict_end=dict_end);
}

func set_withdrawal_sent_to_L1{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(
   withdrawal_id: felt
) {
    alloc_locals;
...
@view
func get_withdrawal_sent_to_L1{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(
   withdrawal_id: felt
) -> (value: felt) {
    alloc_locals;
    let (local word_index, local bit_index) = find_bit(withdrawal_id);
